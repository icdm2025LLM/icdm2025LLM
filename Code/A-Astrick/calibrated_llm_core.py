...
class CalibratedRLTrainer:
    ...
    def train_unguided_rl(self, env_width=4, env_height=4):
        ...
        # Create separate directory for this run
        save_dir = os.path.join(config.SAVE_DIR, f"unguided_rl_logs/{env_width}x{env_height}")
        ...
        for episode in range(num_episodes):
            ...
            print(f"Episode {episode+1}/{num_episodes} | Reward: {episode_reward:.2f} | Steps: {step_count} | "
                f"Key: {'✅' if key_picked else '❌'}, Door: {'✅' if door_opened else '❌'}, Goal: {'✅' if goal_reached else '❌'}")
        ...
        print(f"✅ Saved unguided policy to {os.path.join(save_dir, 'policy.pt')}")

        # Save CSV log
        df = pd.DataFrame(log_rows)
        df.to_csv(os.path.join(save_dir, "training_log.csv"), index=False)
        print(f"📄 Saved training log to: {os.path.join(save_dir, 'training_log.csv')}")

        # AUC
        rewards = [row["reward"] for row in log_rows]
    ...
    def train_linear_decay_rl(self, env_width=4, env_height=4):
        ...
        # Create separate directory for this run
        save_dir = os.path.join(config.SAVE_DIR, f"linear_decay_logs/{env_width}x{env_height}")
        os.makedirs(save_dir, exist_ok=True)
        ...
        for episode in range(num_episodes):
            ...
            episode_rewards.append(episode_reward)
            episode_lengths.append(step_count)
            entropy_values.append(linear_coef)
            completed_tasks.append([key_picked, door_opened, goal_reached])
        ...
        print(f"✅ Saved linear decay policy to {model_path}")
